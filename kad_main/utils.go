...
const (
	...
	TestNodeSize int = 100 // Nodes are numbered with 0 ~ 100 (101 nodes in total).

	NodeSize                   int     = 50
	forceQuitPutSize           int     = 500
	RoundNum                   int     = 10
	forceQuitMaxFailRate       float64 = 0.15
	forceQuitRoundQuitNodeSize = NodeSize / RoundNum
	AfterRunSleepTime           = 200 * time.Millisecond
	JoinSleepTime        = time.Second
	AfterJoinSleepTime = 10 * time.Second
	forceQuitSleepTime = 500 * time.Millisecond
)
